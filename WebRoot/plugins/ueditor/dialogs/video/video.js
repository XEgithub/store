...
//        }
    ...
    function initButtons() {
        ...
        dialog.onok = function () {
            var list = [], id, tabs = $G('tabHeads').children;
            for (var i = 0; i < tabs.length; i++) {
            ...
            pushData: function (list) {
                ...
                for (i = 0; i < list.length; i++) {
                    if(list[i] && list[i].url) {
                        ...
                        item.setAttribute('data-url', getRootPath()+urlPrefix + list[i].url);
                        if (list[i].original) {
                    	...
                    	item.appendChild($("<span class='delbtn' url='" + list[i].url + "'>✖</span>").click(function (){
        					...
        					if (!confirm("确定要删除吗？")) return;
        					  var url = editor.getActionUrl(editor.getOpt('imageDelUrl')),isJsonp = utils.isCrossDomainUrl(url);
        					  $.ajax({
        					      type:      'post',
        					      url:      url,
        					      data:      {"path": del.attr("url")},
        					      dataType:    'jsonp',
   ...
   function initVideo(){
    ...
    function insertVideo(id){
      ...
      var liDatas = domUtils.getElementsByTagName($G(id),"li"),
      videoObjs=[];
       for(var i=0,img; img=liDatas[i++];){ 
...
//        return list;
    ...
    function insertUpload(){
        ...
        for(var key in uploadVideoList) {
            ...
            videoObjs.push({
                url: getRootPath()+uploadDir + file.url,
                width:width,
                height:height,
                align:align
            });
