...
import com.fh.service.information.news.NewsManager;
...
public class NewsController extends BaseController {
	...
	public ModelAndView exportExcel() throws Exception{
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var4", varOList.get(i).getString("PUBLISHER"));	    //4
			vpd.put("var5", varOList.get(i).getString("ADDTIME"));	    //5
			vpd.put("var6", varOList.get(i).getString("UPTIME"));	    //6
			vpd.put("var7", varOList.get(i).getString("PIP"));	    //7
			vpd.put("var8", varOList.get(i).get("HITS").toString());	//8
			vpd.put("var9", varOList.get(i).get("SEQUENCE").toString());	//9
			vpd.put("var10", varOList.get(i).getString("RECOMMAND"));	    //10
