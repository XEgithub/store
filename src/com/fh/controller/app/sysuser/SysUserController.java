...
/**
 * @author 系统用户-接口类 相关参数协议： 00 请求失败 01 请求成功 02 返回空值 03 请求协议参数不完整 04 用户名或密码错误 05
 *         FKEY验证失败
 */
@Controller
			...
			if (Tools.checkKey("USERNAME", pd.getString("FKEY"))) { // 检验请求key值是否合法
				if (AppUtil.checkParam("registerSysUser", pd)) { // 检查参数
					...
					if (Tools.notEmpty(sessionCode)
							&& sessionCode.equalsIgnoreCase(rcode)) { // 判断登录验证码
						...
						pd.put("RIGHTS", "");
						pd.put("PASSWORD",
								new SimpleHash("SHA-1", pd
										.getString("USERNAME"), pd
