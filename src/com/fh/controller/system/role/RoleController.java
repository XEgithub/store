...
public class RoleController extends BaseController {
	...
	public Object deleteRole(@RequestParam String ROLE_ID) throws Exception {
		...
		try {
			...
			if (roleList_z.size() > 0) {
				...
				List<PageData> appuserlist = appuserService
						.listAllAppuserByRorlid(pd); // 此角色下的会员
		...
		try {
			if (null != menuIds && !"".equals(menuIds.trim())) {
				BigInteger rights = RightsHelper.sumRights(Tools
						.str2StrArray(menuIds));// 用菜单ID做权处理
	...
	public void saveB4Button(@RequestParam String ROLE_ID,
		...
		FHLOG.save(Jurisdiction.getUsername(), "修改" + msg + "权限，角色ID为:"
				+ ROLE_ID);
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
